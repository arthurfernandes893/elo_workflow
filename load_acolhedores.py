        ...
        with open(caminho_csv, mode='r', encoding='utf-8') as file:
            ...
            for i, linha in enumerate(leitor_csv, start=2): # start=2 para contar a linha do cabeçalho
                ...
                if not nome or not email or not lider_gps_nome:
                    print(f"AVISO (Linha {i}): Linha incompleta. Todos os campos são obrigatórios. Ignorando.")
                    logs['erro_linha'] += 1
                    continue

                # Passo 1: Encontrar o id_gps correspondente ao nome do líder
                cursor.execute("SELECT id_gps FROM gps WHERE nome_lider_gps = ?", (lider_gps_nome,))
                resultado_gps = cursor.fetchone()

                if resultado_gps is None:
                    print(f"ERRO (Linha {i}): Grupo com líder '{lider_gps_nome}' não encontrado no banco de dados. Ignorando acolhedor '{nome}'.")
                    logs['erro_gps'] += 1
                    continue
                ...
                try:
                    ...
                    print(f"AVISO (Linha {i}): Acolhedor com e-mail '{email}' já existe no banco. Ignorando.")
                    logs['ja_existe'] += 1
